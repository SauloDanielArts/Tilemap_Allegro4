...
int main() {
	...
	BITMAP *Buffer = create_bitmap(SCREEN_WIDTH+200,SCREEN_HEIGHT+200);
     ...
     Longit_Lat();//latitude e longitude do mapa em relacao ao ponto 0x0 da tela
		...
		//---criar uma funcao chamada fase'x' com o modelo que esta representado abaixo---  
		 ...
		 // obijeto de test-----------
		  ...
		  rect(Buffer, xc+100,yc+100, xc+64+100, yc+64+100, makecol(0, 0, 100));
		  rectfill(Buffer, xc+100,yc+100, xc+64+100, yc+64+100, makecol(0, 0, 255));
		  drawing_mode(DRAW_MODE_SOLID, 0, 0, 0); 
		  
		if(mouse_b & 1)
		  xc=mouse_x-8+100,yc=mouse_y-8+100;
        ...
        rect(Buffer, 100,100, 640+100,480+100, makecol(255, 255, 0));     
  
		  
	  vsync();//aguarda a sincronizacao vertical
	  rest(80);//espera 'x' milisegundos	  
	  blit(Buffer,screen,0,0,0,0,SCREEN_WIDTH+200,SCREEN_HEIGHT+200);//fixa o Buffer no screen	  
...
void init() {
	...
	res = set_gfx_mode(GFX_AUTODETECT_WINDOWED, SCREEN_WIDTH +200,SCREEN_HEIGHT +200, 0, 0);//SCREEN_WIDTH ,SCREEN_HEIGHT
	if (res != 0) {
