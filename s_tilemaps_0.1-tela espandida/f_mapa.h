        ...
        void Fill_Map(BITMAP *till[],int Nt,int *map_e,int M_X,int M_Y, BITMAP *Buffer_externo,float dx ,float dy)
                  {//(Mx=colunas)(My=linhas)
                    ...
                    Ymax=Y_MAP*dy;
                     ...
                     //rect(Buffer_externo, Xmax+100, Ymax+100, M_X,M_Y, makecol(0, 255, 0));  
					...
					for(i=0;i<M_X;i++)//inprime o tile map
                       {
	                    ...
	                    for(k=1;k<Nt;k++)                  
							if(k==map_e[P]){
                              draw_sprite(Buffer_externo, till[k-1], Xmax+100, Ymax+100);  
						}	
                           P++; 
                           cont++;
                              Xmax=Xmax+Tx_Tile;
        ...
        void Fill_Map(BITMAP *till[],int Nt,int *map_e,int M_X,int M_Y, BITMAP *Buffer_externo)
				...
				for(j=0;j<M_Y;j++)
					...
					for(i=0;i<M_X;i++)//inprime o tile map
                      ...
                      //rect(Buffer_externo, Xmax+100, Ymax+100, M_X*Tx_Tile,M_Y*Tx_Tile, makecol(0, 255, 0)); 
                  }                  
					...
					for(i=0;i<M_X;i++)//inprime o tile map
                       {
	                    ...
	                    //for(k=1;k<Ct;k++)                  
							if(map_e[P]>0){//(k==map_e[P])
                              //draw_sprite(Buffer_externo, till[k-1], Xmax, Ymax); 
                              rect(Buffer_externo, Xmax+100, Ymax+100, Xmax+Tx_Tile-1+100, Ymax+Ty_Tile-1+100, makecol(200, 0, 0));         
						      tile.x =Xmax;
	                          tile.y =Ymax;
                              ...
                              if(Colisao(tile, obj))
                                 {
                                  ...
                                  rectfill(Buffer_externo, Xmax+100, Ymax+100, Xmax+Tx_Tile-1+100, Ymax+Ty_Tile-1+100, makecol(255, 0, 0));
                                 ...
                                 if (tile.y+100 < obj.y + obj.a+100 && obj.x+obj.l+100>tile.x-1+100 && obj.x+100<tile.x+Tx_Tile+100 && obj.y+100>tile.y+100) obj_y=Ymax+(tile.a);  
                                
                                 //obj está acima do tile
                                 if (obj.y+100 + obj.a+100 > tile.y-1+100 && obj.x+obj.l+100>tile.x-1+100 && obj.x+100<tile.x+Tx_Tile+100 && obj.y+100<tile.y+100) obj_y=Ymax-(obj.a+1);                              
